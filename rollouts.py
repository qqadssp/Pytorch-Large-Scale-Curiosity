...
class Rollout(object):
    def __init__(self, ob_space, ac_space, nenvs, nsteps_per_seg, nsegs_per_env, nlumps, envs, policy,
        ...
        self.acs_record = [None] * self.nlumps
    ...
    def rollout_step(self):
        ...
        for l in range(self.nlumps):
            ...
            if self.step_count == 0:
                ...
                obs, prevrews, news, infos = (ob, None, np.ones(self.lump_stride, bool), {})
            else:
...
#            obs, prevrews, news, infos = self.env_get(l)
        ...
        if s == self.nsteps_per_seg - 1:
            for l in range(self.nlumps):
                sli = slice(l * self.lump_stride, (l + 1) * self.lump_stride)
                nextobs, ext_rews, nextnews, _ = self.envs[l].step_async(self.acs_record[l])
    ...
    def update_info(self):
        all_ep_infos = self.ep_infos_new # MPI.COMM_WORLD.allgather()
#        all_ep_infos = sorted(sum(all_ep_infos, []), key=lambda x: x[0]) # it will not work with this line, I don't know why.
        if all_ep_infos:
