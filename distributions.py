...
class CategoricalPd(Pd):
    ...
    def neglogp(self, x):
        nlp = torch.nn.functional.cross_entropy(input=self.logits, target=x.squeeze(), reduction='none') # input = [N, [nprob]], target = [N], one_hot encoding is not needed
        return nlp.unsqueeze(1) # keep the same output shape as the function sample and entropy
    ...
    def sample(self):
        ...
        return torch.argmax(self.logits - torch.log(-torch.log(u)), dim=-1, keepdim=True) # not really understand this formula
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)
'''
